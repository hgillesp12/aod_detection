...
def export_metrics_to_csv(save_path, tp, fp, p, r, f1, ap, unique_classes, px, py):
    """
    Exports evaluation metrics to a CSV file for plotting purposes.

    Args:
        save_path (str | Path): Path to the CSV file where the metrics will be saved.
        tp (ndarray): True Positives for each class.
        fp (ndarray): False Positives for each class.
        p (ndarray): Precision values for each class.
        r (ndarray): Recall values for each class.
        f1 (ndarray): F1 scores for each class.
        ap (ndarray): Average Precision (AP) for each class.
        unique_classes (ndarray): Array of unique class indices.
        px (ndarray): X-coordinates for plotting (recall levels).
        py (list of ndarray): Y-coordinates for plotting (precision levels per class).
    ...
    with open(save_path, mode='w', newline='') as file:
        ...
        # Write the header
        writer.writerow(['Class', 'True Positives', 'False Positives', 'Precision', 'Recall', 'F1 Score', 'AP'])

        # Write the data for each class
        for i, c in enumerate(unique_classes):
            writer.writerow([c, int(tp[i]), int(fp[i]), p[i], r[i], f1[i], ap[i]])

        # Write precision-recall curve data for plotting
        writer.writerow([])
        writer.writerow(['Recall'] + [f'Class {c} Precision' for c in unique_classes])
        for i in range(len(px)):
            row = [px[i]] + [py[j][i] if j < len(py) else '' for j in range(len(unique_classes))]
            writer.writerow(row)
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir=".", names=(), eps=1e-16, prefix=""):
    ...
    save_path = '/content/drive/My Drive/Imperial College London/Thesis/data/validation_results/results_augmented_combined.csv'
    
    # Export metrics for plotting
    export_metrics_to_csv(save_path, tp, fp, p, r, f1, ap, unique_classes, px, py)
